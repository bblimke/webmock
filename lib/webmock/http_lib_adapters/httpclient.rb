if defined?(HTTPClient)
  ...
  class HTTPClient
    ...
    def do_get_stream_with_webmock(req, proxy, conn, &block)
      ...
      if WebMock.registered_request?(request_signature)
        webmock_response = WebMock.response_for_request(request_signature)
        response = build_httpclient_stream_response(webmock_response, &block)
        conn.push(response)
      elsif WebMock.net_connect_allowed?
      ...
      while retry_count > 0
        ...
        begin
          protect_keep_alive_disconnected do
            do_get_stream_with_webmock(req, proxy, tconn)
          end
          break
    ...
    def build_httpclient_stream_response(webmock_response, &block)
      response = HTTP::Message.new_response(StringIO.new(webmock_response.body))
      response.header.init_response(webmock_response.status)
      # response.body =  HTTP::Message::Body.new
      #response.body.init_response(StringIO.new(webmock_response.body))
